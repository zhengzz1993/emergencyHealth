...
export default {
  ...
  watch: {
    'inputForm.dataSet.id' (val) {
      if (val) {
        this.$http.get(`/database/datamodel/dataMeta/queryNeedByDataSetId?id=${val}`).then(({data}) => {
          this.metas = data.metas.filter((item) => {
  ...
  methods: {
    init (method, id) {
      ...
      this.$nextTick(() => {
        ...
        this.$http.get('/database/datamodel/dataSet/list', {params: {pageSize: -1}}).then(({data}) => {
          this.dataSets = data.page.list
        })
        if (method === 'edit' || method === 'view') { // 修改或者查看
          this.$http({
            url: `/echarts/queryDesignById?id=${this.inputForm.id}`,
            method: 'get'
          }).then(({data}) => {
    ...
    refreshChart () {
      if (this.inputForm.dataSet.id && this.inputForm.xNames && this.inputForm.yNames) {
        this.$http.get(`/echarts/mergeChartData`,
          { params: {
