  ...
  <div class="page">
    ...
    <el-table
      ...
      <el-table-column
        ...
        label="员工姓名">
            <template slot-scope="scope">
              <el-link  type="primary" :underline="false" v-if="hasPermission('test:activiti:testActivitiExpense:edit')" @click="edit(scope.row.id)">{{scope.row.user.name}}</el-link>
              <el-link  type="primary" :underline="false" v-else-if="hasPermission('test:activiti:testActivitiExpense:view')"  @click="view(scope.row.id)">{{scope.row.user.name}}</el-link>
    ...
    // 排序
     ...
     // 查看详情
      ...
      exportExcel () {
        ...
        this.$utils.download('/test/activiti/testActivitiExpense/export', params)
      },
      resetSearch () {
