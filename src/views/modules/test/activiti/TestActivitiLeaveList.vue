<template>
  <div class="page">
        ...
        <el-dialog  title="导入Excel" :visible.sync="isImportCollapse">
          <el-form :inline="true" v-show="isImportCollapse"  class="query-form" ref="importForm">
             ...
             <el-form-item prop="loginName">
                <el-upload
                  class="upload-demo"
                  :action="`${this.$http.BASE_URL}/test/activiti/testActivitiLeave/import`"
      ...
      <el-row>
        ...
        <el-button v-if="hasPermission('test:activiti:testActivitiLeave:edit')" type="warning" size="small" icon="el-icon-edit-outline" @click="edit()"
         :disabled="dataListSelections.length != 1" plain>修改</el-button>
        <el-button v-if="hasPermission('test:activiti:testActivitiLeave:del')" type="danger"   size="small" icon="el-icon-delete" @click="del()"
                  :disabled="dataListSelections.length <= 0" plain>删除
        </el-button>
        <el-button-group class="pull-right">
            <el-button v-if="hasPermission('test:activiti:testActivitiLeave:import')" type="default" size="small" icon="el-icon-upload2" title="导入" @click="isImportCollapse = !isImportCollapse, isSearchCollapse=false"></el-button>
            <el-button v-if="hasPermission('test:activiti:testActivitiLeave:export')" type="default" size="small" icon="el-icon-download" title="导出" @click="exportExcel()"></el-button>
            <el-button
              type="default"
              size="small"
...
<script>
  ...
  export default {
    ...
    // 排序
     ...
     // 查看详情
      detail (row) {
        this.$http.get(`/test/activiti/testActivitiLeave/queryById?id=${row.id}`).then(({data}) => {
      ...
      // 下载模板
      downloadTpl () {
        this.$utils.download('/test/activiti/testActivitiLeave/import/template')
      },
      exportExcel () {
        let params = {
          ...this.searchForm
        }
        this.$utils.download('/test/activiti/testActivitiLeave/export', params)
      },
      resetSearch () {
