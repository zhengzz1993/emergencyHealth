  ...
  <el-dialog
    ...
    <el-form :model="inputForm" size="small" ref="inputForm" v-loading="loading" :class="method==='view'?'readonly':''"  :disabled="method==='view'"
      ...
      <el-row  :gutter="15">
        ...
        <el-col :span="12">
            <el-form-item label="汉子" prop="zn"
                :rules="[
                  {validator: validator.isChinese, trigger:'blur'}
                 ]">
              <el-input v-model="inputForm.zn" placeholder="请填写汉子"     ></el-input>
           </el-form-item>
        </el-col>
        <el-col :span="12">
      ...
      init (method, id) {
        ...
        this.$nextTick(() => {
          ...
          if (method === 'edit' || method === 'view') { // 修改或者查看
            this.loading = true
            this.$http({
              url: `/test/validation/testValidation/queryById?id=${this.inputForm.id}`,
              method: 'get'
            }).then(({data}) => {
              this.inputForm = this.recover(this.inputForm, data.testValidation)
              this.loading = false
            })
          }
        })
